...
def yolo_forward(output, conf_thresh, num_classes, anchors,
...
def artic_forward_dynamic( output, conf_thresh, num_classes,
  anchors, num_anchors, scale_x_y, only_objectness=1, validation=False ):
    ...
    # Apply sigmoid() and tanh to slices
    bxy = torch.sigmoid(bxy) * scale_x_y - 0.5 * (scale_x_y - 1)
    bwh = torch.tanh(bwh)
...
class ArticLayer(nn.Module):
    ...
    def __init__(self, anchor_mask=[], num_classes=0, anchors=[], num_anchors=1, stride=32, model_out=False):
        super(ArticLayer, self).__init__()
